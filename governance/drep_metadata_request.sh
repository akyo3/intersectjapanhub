...
# 指定したディレクトリ内のすべての -metadata.json ファイルを一つにまとめて整形し、角括弧を取り除く
jq -s '.' $HOME/drep-workspace/metadata/*-metadata.json | jq -c '.[]' | jq -s add > $HOME/drep-workspace/metadata/combined_metadata.json